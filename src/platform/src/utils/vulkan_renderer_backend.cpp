...
void QtVulkanRenderableResource::createPlatformSurface() {
    ...
    qDebug() << "QtVulkanRenderableResource::createPlatformSurface() - skipping surface creation for Qt Quick "
                "offscreen rendering";
...
void QtVulkanRenderableResource::createOffscreenRenderPass() {
    ...
    const auto colorAttachmentReference = vk::AttachmentReference().setAttachment(0).setLayout(
...
void VulkanRendererBackend::ensureInitialized() {
    ...
    if (window->width() <= 0 || window->height() <= 0) {
        qWarning() << "Window has invalid dimensions during initialization:" << window->width() << "x"
                   << window->height();
...
void VulkanRendererBackend::createOffscreenTexture(const mbgl::Size& newSize) {
    if (context && !newSize.isEmpty()) {
        qDebug() << "Creating offscreen texture for VulkanRendererBackend with size:" << newSize.width << "x"
                 << newSize.height;
